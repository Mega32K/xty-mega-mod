...
import com.mega.xty.common.data.fps.PlayerNameData;
...
public class S2CPlayerNamePacket {
    ...
    private final Map<UUID, PlayerNameData> dataMap;

    public S2CPlayerNamePacket(boolean differentSave, Map<UUID, PlayerNameData> dataMap) {
        this.differentSave = differentSave;
        this.dataMap = dataMap;
    }

    public static S2CPlayerNamePacket decode(FriendlyByteBuf friendlyByteBuf) {
        return new S2CPlayerNamePacket(friendlyByteBuf.readBoolean(), friendlyByteBuf.readMap(FriendlyByteBuf::readUUID, PlayerNameData.F_READER));
    }
    ...
    public static void encode(S2CPlayerNamePacket packet, FriendlyByteBuf friendlyByteBuf) {
        ...
        friendlyByteBuf.writeMap(packet.dataMap, FriendlyByteBuf::writeUUID, PlayerNameData.F_WRITER);
    }
    ...
    static void handle0(S2CPlayerNamePacket packet, Supplier<NetworkEvent.Context> context) {
        if (context.get().getDirection() == NetworkDirection.PLAY_TO_CLIENT) {
            ...
            for (var entry : packet.dataMap.entrySet())
                ClientFpsData.playerDisplayNames.put(entry.getKey(), entry.getValue().component.copy());
        }
    }
}
