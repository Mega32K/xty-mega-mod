package com.mega.map.common.item;
...
import com.mega.map.client.overlay.DebugModules;
    ...
    public void appendHoverText(@NotNull ItemStack itemStack, @Nullable Level level, @NotNull List<Component> lines, @NotNull TooltipFlag tooltipFlag) {
        ...
        if (nbt != null) {
            ...
            if (!interaction.isEmpty()) {
                ...
                if (start != null)
                    lines.add(Component.translatable("item.megamod.fill_function_creator.lore.selected_start", LoreHelper.blockPos(start)).withStyle(ChatFormatting.GRAY));
                if (end != null)
                    lines.add(Component.translatable("item.megamod.fill_function_creator.lore.selected_end", LoreHelper.blockPos(end)).withStyle(ChatFormatting.GRAY));
            }
            FillCreatorComponent component = ItemComponentManager.get(itemStack, ComponentInit.FILL_CREATOR);
            if (component != null) {
                if (component.use().isEmpty()) {
                    lines.add(Component.translatable("item.megamod.fill_function_creator.lore.use").withStyle(ChatFormatting.GRAY));
                }
            }
        }
    }
